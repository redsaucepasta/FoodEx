...
mongoose.connect(
  "mongodb+srv://admin-amit:sonunandini@cluster0.k902j.mongodb.net/FoodEx",
                ...
                if (!err) {
                  ...
                  if (foundCart.outlet == "") {
                    ...
                    Cart.findOneAndUpdate(
                      { _id: foundUser._id },
                      { $push: { item: detailsArray }, outlet: outlet },
                      function (err, succ) {
                        if (err) {
                          console.log(err);
                        } else {
                    ...
                    Cart.findOne(
                      ...
                      function (err, foundCart) {
                          ...
                          existingItems.forEach(function (existingItem) {
                            if (existingItem.name === item.name) {
                              ...
                              existingItems.splice(
                                existingItems.indexOf(existingItem),
                                1
                              );
                            }
                          });
                        ...
                        Cart.findOneAndUpdate(
                          { _id: foundUser._id },
                          { item: detailsArray, outlet: outlet },
                          function (err, succ) {
                            if (err) {
                              console.log(err);
                            } else {
    ...
    User.findOne({ username: req.user.username }, function (err, foundUser) {
      if (err) {
        ...
        Cart.findOne({ _id: foundUser._id }, function (err, foundCart) {
          ...
          Cart.findOneAndUpdate(
            { _id: foundUser._id },
            { total: total, outlet: outlet },
            function (err, foundCart) {
              Cart.findOne({ _id: foundUser._id }, function (err, foundCart) {
                res.render("cart", {
                  carttotal: foundCart.total,
                  cart: foundCart.item,
                  outlet: foundCart.outlet,
                });
              });
...
app.listen(process.env.PORT || PORT, function () {
  console.log("FoodEx Customer Server\nServer started on port:", PORT);
});
